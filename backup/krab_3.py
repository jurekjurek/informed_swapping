...
def resolve_Q(Q: QSchedule, application: int) -> int:
    """
    Resolve the number of newly accepted candidate bitstrings for a given
    ...
    Accepted forms:
        ...
        Q = [500, 400, 300, 200, 100]
            use Q[application]; after the list ends, keep using the final value.

        Q = lambda application: max(50, int(500 * 0.8**application))
            arbitrary schedule.
    """
    if callable(Q):
    ...
    if q_value < 0:
        raise ValueError(f"Q must be nonnegative, got Q={q_value}.")
...
def project_hamiltonian_to_subspace(
    ...
    """
    Build H_S = P_S H P_S in the ordering given by active.
    """
    m = len(active)
...
def diagnose_state_sparsity(
    ...
    *,
    ...
    pr = float(1.0 / ipr) if ipr > 0 else 0.0
...
def apply_max_subspace_cap(
    ...
    *,
    ...
    """
    Enforce |S| <= max_subspace_dim using composite score.
    ...
    protected_indices can be used for the seed in the initial step.
    """
    ...
    if len(protected) > max_subspace_dim:
        protected = sorted(protected, key=lambda idx: first_acceptance_rank[idx])[:max_subspace_dim]
...
def selected_krylov_ground_state(
    ...
    *,
    ...
    Selected-subspace Krylov-like ground-state search with:
        ...
        1. candidate pruning before diagonalization:
               keep up to Q candidates with |r_a| >= delta;

        2. coefficient pruning after diagonalization:
               keep states with |c_b|^2 >= epsilon by default;

        3. hard active-space cap:
    ...
    initial_diagnostics = InitialDiagnostics(
        ...
        n_selected_from_initial_application=min(Q, len(h_seed)),
    ...
    for it in range(n_iterations):
        ...
        # Candidate selection using r_a = <a|H|psi>.
        ...
        # Enforce hard maximum active-space dimension.
    ...
    if final_cleanup_prune:
        for _ in range(max_final_cleanup_passes):
            ...
            cleaned_active, cleaned_coeffs, n_pruned, _, _ = prune_active_by_coefficients(
                active=active,
                coeffs=final_coeffs,
                epsilon=epsilon,
                min_keep=min_keep_after_prune,
